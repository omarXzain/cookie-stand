...
function footer() {
  ...
  for (var h = 0; h<hours.length;h++){
    totalHours = 0;
    for(var x = 0; x < omarStore.length; x++){
      totalHours += omarStore[x].cookiesArr[h];
...
header();
for (var i = 0; i <omarStore.length;i++){
  omarStore[i].render();
}
footer();
...
var locationForm = document.getElementById('former');
locationForm.addEventListener('submit', function (event){
  event.preventDefault();
  var locationInput = event.target.NameX.value;
  var minInput = event.target.minX.value;
  var maxInput = event.target.maxX.value;
  var avgCoookiesInput = event.target.avgX.value;
